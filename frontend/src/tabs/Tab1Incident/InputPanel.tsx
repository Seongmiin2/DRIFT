...
export function InputPanel() {
  ...
  return (
    <form
      ...
      className="flex flex-col gap-4 p-4 bg-gradient-to-b from-navy-900 to-navy-950 border-r border-navy-700/70 w-72 shrink-0 overflow-y-auto"
    >
      <div className="flex items-center gap-2 pb-3 border-b border-navy-700/60">
      ...
      <div className="flex flex-col gap-3 p-3 bg-navy-800/40 rounded-lg border border-navy-700/50">
        <div className="flex flex-col gap-1">
          <label className="section-label">선박 ID (V-Pass)</label>
          <input
            type="text"
        ...
        <div className="flex flex-col gap-1">
          <label className="section-label">선종 *</label>
          <select
      ...
      <div className="flex flex-col gap-3 p-3 bg-navy-800/40 rounded-lg border border-navy-700/50">
        <div className="grid grid-cols-2 gap-2">
          <div className="flex flex-col gap-1">
            <label className="section-label">경도 (°E) *</label>
            <input
              type="number"
              step="0.001"
              min="123"
          ...
          <div className="flex flex-col gap-1">
            <label className="section-label">위도 (°N) *</label>
            <input
              type="number"
              step="0.001"
              min="32"
        ...
        <div className="flex flex-col gap-1">
          <label className="section-label">최종 확인 시각</label>
          <input
            type="datetime-local"
            value={
      ...
      <div className="flex flex-col gap-3 p-3 bg-navy-800/40 rounded-lg border border-navy-700/50">
        <div className="flex flex-col gap-1">
          <label className="section-label">총톤수 (GT)</label>
          <input
            type="number"
            step="0.1"
            min="0"
        ...
        <div className="flex flex-col gap-1">
          <label className="section-label">현장 메모</label>
          <textarea
            rows={2}
      ...
      {error && (
        <p className="text-xs text-red-400 bg-red-400/10 border border-red-400/20 rounded-md p-2">{error}</p>
      )}
      ...
      <button
        ...
        className={[
          "mt-auto w-full py-2.5 rounded-md text-sm font-semibold tracking-wide transition-all duration-200",
          isSubmitting
            ? "bg-navy-700 text-slate-500 cursor-not-allowed"
            : "bg-gradient-to-r from-cyan-400 to-cyan-300 text-navy-950 hover:from-cyan-300 hover:to-cyan-200 shadow-glow hover:shadow-[0_0_28px_rgba(0,212,255,0.5)] hover:scale-[1.01] active:scale-[0.99]",
        ].join(" ")}
      >
        {isSubmitting ? (
