...
try:
    from engine.data_fetcher import EnvironmentData, fetch_environment
...
class MockEngine:
    ...
    def predict(self, request: PredictionRequest) -> EnginePredictionResult:
        ...
        if env is not None:
            ...
            wind_dir    = env.weather.wind_direction_deg
            wind_ms     = env.weather.wind_speed_ms
            data_source_current = env.current.source
            data_source_weather = env.weather.source
            data_freshness_ok   = env.data_freshness_ok
        else:
